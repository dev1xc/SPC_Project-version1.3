...
export const ModalCitizenID = ({ closeModal, onSubmit, defaultValue }) => {
    ...
    const validateForm = () => {
        if(formState.citizen_id && formState.citizenIdNumber && formState.issueDate && formState.issuePlace && formState.email && formState.nation && formState.occupation && formState.relationship && formState.income){
            setErrors("")
            return true;
        } else {
            let errorFields = [];

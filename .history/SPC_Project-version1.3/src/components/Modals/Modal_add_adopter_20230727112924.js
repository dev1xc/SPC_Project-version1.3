...
export const ModalAdopters = ({ closeModal, onSubmit, defaultValue }) => {
    ...
    const validateForm = () => {
        if(formState.citizen_id && formState.adopter_id && formState.birthday && formState.phoneNumber && formState.email && formState.nation && formState.occupation && formState.relationship){
            setErrors("")
            return true;
        } else {
            let errorFields = [];
